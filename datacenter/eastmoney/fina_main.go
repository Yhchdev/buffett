...
func (h HistoricalFinaMainData) FilterByReportType(reportType FinaReportType) HistoricalFinaMainData {
	...
	sort.Slice(result, func(i, j int) bool {
		return result[i].ReportYear < result[j].ReportYear
	})
...
func (h HistoricalFinaMainData) ValueList(
	...
	r := []float64{}
	data := h.FilterByReportType(reportType)
	dataLen := len(data)
	if dataLen == 0 {
		return r
	}
...
func (h HistoricalFinaMainData) IsStability(
	...
	if err != nil {
		logrus.Error(ctx, "IsStability StdDeviationFloat64 error:"+err.Error())
		return false
	}
	logrus.Debugf(ctx, "StdDeviation value:%v", sd)
...
func (e EastMoney) QueryHistoricalFinaMainData(ctx context.Context, secuCode string) (HistoricalFinaMainData, error) {
	...
	logrus.Debug(ctx, "EastMoney QueryHistoricalFinaMainData "+apiurl+" begin", zap.Any("params", params))
	beginTime := time.Now()

	finaMain := &RespFinaMainData{}
	//err = e.HTTPGET(ctx, e.HTTPClient, apiurl, nil, &resp)
...
func (e EastMoney) QueryFinaPublishDateList(ctx context.Context, securityCode string) (FinaPublishDateList, error) {
	...
	logrus.Debug(ctx, "EastMoney QueryFinaPublishDate "+apiurl+" begin", zap.Any("params", params))
	beginTime := time.Now()
